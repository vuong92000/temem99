...
try {
  upstream = http.createServer((req, res) => {
    if (req.method === 'POST' && req.url === '/api/tasks/simple') {
      let raw = '';
      req.on('data', chunk => { raw += chunk; });
      req.on('end', () => {
        received = new URLSearchParams(raw);
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ task_id: 'mock-task-1' }));
      });
      return;
    }
    if (req.method === 'GET' && req.url === '/api/tasks/mock-task-1') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ task_id: 'mock-task-1', status: 'completed', progress: 1 }));
      return;
    }
    if (req.method === 'GET' && req.url === '/api/video/mock-task-1') {
